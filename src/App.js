...
const AIAssistant = ({ topicTitle, topicExplanation }) => {
  const [question, setQuestion] = useState('');
  const [answer, setAnswer] = useState('');
  const [loading, setLoading] = useState(false);
  const answerRef = useRef(null);
  const [aiMode, setAiMode] = useState('ask'); // 'ask', 'suggest_topics', 'suggest_pyqs', 'explain_code', 'generate_code'

  const getPrompt = (mode, currentTopicTitle, currentQuestion = '') => {
    switch (mode) {
      case 'ask':
        return `You are an expert Java OOP tutor. The current topic is "${currentTopicTitle}". Here's the explanation:
"""
${topicExplanation}
"""
Based on this context and your general knowledge, please answer the following question concisely and clearly, using Markdown for formatting (e.g., **bold**, *italics*, code snippets):
"${currentQuestion}"`;
      case 'suggest_topics':
        return `You are an expert Java OOP tutor. The current topic is "${currentTopicTitle}". Suggest 3-5 advanced or closely related OOP topics that a learner might want to explore next, providing a brief one-sentence description for each. Format your response as a Markdown unordered list.`;
      case 'suggest_pyqs':
        return `You are an expert Java OOP tutor. The current topic is "${currentTopicTitle}". Generate 3-5 challenging previous year questions (PYQs) related to this topic. Provide a brief hint or key concept to consider for each. Format your response as a Markdown unordered list.`;
      case 'explain_code':
        return `You are an expert Java OOP tutor. Explain the following Java code snippet concisely and clearly. Focus on its OOP concepts and functionality. Use Markdown for formatting (e.g., **bold**, *italics*, code snippets):
\`\`\`java
${currentQuestion}
\`\`\``;
      case 'generate_code':
        return `You are an expert Java OOP tutor. Generate a concise Java code example that demonstrates the concept of "${currentQuestion}" within the context of "${currentTopicTitle}". Provide a brief explanation of the code. Use Markdown for formatting and include the code in a Java code block.`;
      default:
        return '';
    }
  };

  const askAI = async (mode = 'ask') => {
    let currentPrompt = '';
    if (mode === 'ask' || mode === 'explain_code' || mode === 'generate_code') {
      if (!question.trim()) return;
      currentPrompt = getPrompt(mode, topicTitle, question);
    } else {
      currentPrompt = getPrompt(mode, topicTitle);
    }
    ...
    const apiKey = process.env.GapiKey || "AIzaSyAX8aGgrubHeusy93Azs9ektKdaYnhoT2E" ; // Use environment variable or fallback
    ...
    try {
      ...
      if (result.candidates && result.candidates.length > 0 &&
        ...
        setAnswer("Sorry, I couldn't generate a response. Please try rephrasing your question or selecting a different option.");
      }
    } catch (error) {
      console.error("Error calling Gemini API:", error);
      setAnswer("An error occurred while connecting to the AI. Please try again later.");
    } finally {
      setLoading(false);
      if (mode !== 'ask') {
        setQuestion(''); // Clear question input after suggesting
      }
    }
  };
  ...
  useEffect(() => {
    // Scroll to the answer when it appears
  ...
  return (
    <div className="bg-gradient-to-r from-purple-50 to-pink-50 p-6 rounded-xl shadow-inner">
      <h3 className="text-2xl font-semibold mb-4 text-purple-700">AI Assistant 🤖</h3>
      <p className="text-gray-700 mb-4">Ask me anything about Java OOP, especially related to the current topic, or ask for suggestions!</p>

      <div className="flex flex-wrap gap-2 mb-4"> {/* Use flex-wrap for responsiveness */}
        <button
          onClick={() => { setAiMode('suggest_topics'); askAI('suggest_topics'); }}
          disabled={loading}
          className="px-3 py-1.5 text-sm bg-gradient-to-br from-purple-700 to-indigo-700 text-white rounded-md shadow-md hover:from-purple-800 hover:to-indigo-800 transition duration-300 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed"
        >
          Suggest Related Topics
        </button>
        <button
          onClick={() => { setAiMode('suggest_pyqs'); askAI('suggest_pyqs'); }}
          disabled={loading}
          className="px-3 py-1.5 text-sm bg-gradient-to-br from-purple-700 to-indigo-700 text-white rounded-md shadow-md hover:from-purple-800 hover:to-indigo-800 transition duration-300 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed"
        >
          Suggest PYQs
        ...
        <button
          onClick={() => { setAiMode('explain_code'); setAnswer(''); setQuestion(''); }} // Clear answer/question for new mode
          disabled={loading}
          className="px-3 py-1.5 text-sm bg-gradient-to-br from-purple-700 to-indigo-700 text-white rounded-md shadow-md hover:from-purple-800 hover:to-indigo-800 transition duration-300 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed"
        >
          Explain Code
        ...
        <button
          onClick={() => { setAiMode('generate_code'); setAnswer(''); setQuestion(''); }} // Clear answer/question for new mode
          disabled={loading}
          className="px-3 py-1.5 text-sm bg-gradient-to-br from-purple-700 to-indigo-700 text-white rounded-md shadow-md hover:from-purple-800 hover:to-indigo-800 transition duration-300 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed"
        >
          Generate Code
        ...
        <button
          ...
          className="px-3 py-1.5 text-sm bg-gray-300 text-gray-800 rounded-md shadow-md hover:bg-gray-400 transition duration-300 ease-in-out"
        >
          Clear Chat
        </button>
      </div>
      ...
      <textarea
        className="w-full p-3 border border-purple-300 rounded-lg focus:ring-2 focus:ring-purple-400 focus:border-transparent transition-all duration-200 resize-y min-h-[80px] text-gray-800"
        placeholder={
          aiMode === 'explain_code'
            ? 'Paste Java code snippet here to get an explanation...'
            : aiMode === 'generate_code'
            ? 'Describe the code example you want (e.g., "a simple class for a Dog")...'
            : `Ask a question about ${topicTitle.split(':')[0]}...`
        }
        value={question}
      ...
      ></textarea>
      <button
        onClick={() => askAI(aiMode)} // Use the current AI mode for the request
        disabled={loading || !question.trim()}
        className="mt-4 px-6 py-3 bg-gradient-to-br from-purple-700 to-indigo-700 text-white rounded-md shadow-md hover:from-purple-800 hover:to-indigo-800 transition duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-opacity-75 disabled:opacity-50 disabled:cursor-not-allowed"
      >
        {loading ? 'Thinking...' : (aiMode === 'explain_code' ? 'Explain Code' : aiMode === 'generate_code' ? 'Generate Code' : 'Ask Question')}
      </button>

      {answer && (
        <div ref={answerRef} className="mt-6 p-4 bg-purple-100 rounded-lg border border-purple-200 shadow-sm">
          <h4 className="font-semibold text-lg text-purple-800 mb-2">AI's Response:</h4>
...
function App() {
  ...
  // Function to scroll to AI Assistant
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 font-sans text-gray-900 antialiased">
      <div className="container mx-auto p-6 lg:p-10">
          ...
          <main className="lg:w-3/4 bg-white p-8 rounded-2xl shadow-xl border border-blue-200">
            {selectedTopic && oopContent[selectedTopic] && (
              <div className="space-y-8">
                ...
                {/* AI Assistant */}
                <div ref={aiAssistantRef}> {/* Attach ref here */}
                  <AIAssistant
                    topicTitle={oopContent[selectedTopic].title}
                    topicExplanation={oopContent[selectedTopic].explanation}
                  />
                </div>
        ...
        {/* Floating Action Button to scroll to AI Assistant */}
        {selectedTopic && oopContent[selectedTopic] && ( // Only show if a topic is selected
          <button
            onClick={scrollToAIAssistant}
            className="fixed bottom-6 right-6 bg-purple-600 text-white p-4 rounded-full shadow-lg hover:bg-purple-700 transition-all duration-300 ease-in-out transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-opacity-75"
            title="Scroll to AI Assistant"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.105A9.775 9.775 0 0112 2c4.97 0 9 3.582 9 8z" />
            </svg>
          </button>
        )}
      </div>
